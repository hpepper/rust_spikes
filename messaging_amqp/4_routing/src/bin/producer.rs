...
const EXHANGE_NAME: &str = "direct_logs";
...
async fn main() -> Result<()> {
     ...
     */
    producer_channel.exchange_declare(EXHANGE_NAME, ExchangeKind::Direct, ExchangeDeclareOptions::default(), FieldTable::default()).await?;

    println!(" [*] Declared exchange {:?}", EXHANGE_NAME);

    let args: Vec<String> = env::args().collect();

    let severity = if args.len() > 1 {
        args[1].clone()
    } else {
        "info".to_string()
    };
